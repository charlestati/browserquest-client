...
class Character extends Entity {
  ...
  animate(animation, speed, count, onEndCount) {
    ...
    if (!(this.currentAnimation && this.currentAnimation.name === 'death')) {
      ...
      if (_.indexOf(oriented, newAnimation) >= 0) {
        newAnimation += `_${o === Types.Orientations.LEFT ? 'right'
          : Types.getOrientationAsString(o)}`;
        this.flipSpriteX = (this.orientation === Types.Orientations.LEFT);
      }

      this.setAnimation(newAnimation, speed, count, onEndCount);
    }
  }
   ...
   * Same as Character.follow but with an auto-attacking behavior.
